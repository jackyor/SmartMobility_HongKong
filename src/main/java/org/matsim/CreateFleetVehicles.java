...
public class CreateFleetVehicles {
	...
	private static final int numberOfVehicles = 2000;
	...
	private static final Path networkFile = Paths.get("scenarios/opt/Network.xml");
	private static final Path outputFile = Paths.get("scenarios/opt/drt_2s2000.xml");
	...
	private void run() {
		...
		Stream<DvrpVehicleSpecification> vehicleSpecificationStream = scenario.getNetwork().getLinks().entrySet().stream()
				...
				.map(entry -> ImmutableDvrpVehicleSpecification.newBuilder()
						.id(Id.create("drt" + i[0]++, DvrpVehicle.class))
